...
document.getElementById("formulario").addEventListener("submit", async (e) => {
  ...
  try {
    ...
    // *** CAMBIO PRINCIPAL: Usar el modal en lugar del mensaje simple ***
    statusEl.textContent = "";
    
    // Llamar a la función del modal (definida en formulario.html)
    if (typeof window.mostrarCodigoFamilia === 'function') {
      window.mostrarCodigoFamilia(hhRef.id);
    } else {
      // Fallback si no existe el modal
      statusEl.innerHTML = `<span class="ok">✅ Guardado exitosamente. Código de familia: <strong>${hhRef.id}</strong></span>`;
      e.target.reset();
      $("hhId").value = hhRef.id;
    }
